...
const updateProduct = async (hash: string, data: CreateProduct): Promise<number> => {
  try {
    const response = await apiTic.post(`/products/${hash}`, data);
...
export { createProduct, listProduct, deleteProduct, updateProduct }
